...
$path = "imagesmmi/".$nama_file;
    if(move_uploaded_file($tmp_file, $path)){ // Cek apakah gambar berhasil diupload atau tidak
      // Jika gambar berhasil diupload, Lakukan :  
      // Proses simpan ke Database
	  $query = mysqli_query($koneksi, "UPDATE tb_narasi SET
								mmi='$nama_file' WHERE Status ='Aktif'"); // query untuk memilih entri data dengan nilai nim terpilih
		$pesan=$_SESSION['message_text'];
	...
	$query = mysqli_query($koneksi, "UPDATE tb_penjelasan SET
								mmi='$nama_file' WHERE OT ='$potong'"); // query untuk memilih entri data dengan nilai nim terpilih						
